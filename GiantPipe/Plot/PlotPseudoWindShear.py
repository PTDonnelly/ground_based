...
def PlotCompositePseudoWindShear(dataset):
    ...
    Nlines = 11 if dataset == '2018May' else 10
    fig, axes = plt.subplots(Nlines, 2, figsize=(8,18), sharey=True)
    iaxes = 0
    ...
    for ifilt in subplot_array:
        if ifilt < 6 or ifilt > 7:
            ...
            axes[iaxes,0].set_xlim(-80,-20)
            axes[iaxes,0].set_xticks([-80, -60, -40, -20])
            axes[iaxes,0].xaxis.set_ticklabels([]) if (iaxes < len(subplot_array)-3) else axes[iaxes,0].tick_params(labelsize=20)
            axes[iaxes,0].set_ylim(-0.7,0.7)
            axes[iaxes,0].tick_params(labelsize=20)
            axes[iaxes,0].set_title(ititle[iaxes]+f"    {wavelength}"+r" $\mu$m", fontfamily='sans-serif', loc='left', fontsize=20)
            ...
            axes[iaxes,1].set_xlim(20,80)
            axes[iaxes,1].set_xticks([20, 40, 60, 80])
            axes[iaxes,1].xaxis.set_ticklabels([]) if (iaxes < len(subplot_array)-3) else axes[iaxes,1].tick_params(labelsize=20)
    ...
    plt.ylabel("Pseudo-shear (m s$^{-1}$ km$^{-1}$)", size=20)
    # Save figure 
