...
def PlotRetrievedTemperature():
    ...
    for iprior in prior:
        retrieval_test = [f"{iprior}_temp_aerosol1-1mu-800mbar-05scale-01_C2H2-C2H6_NH3_retrieval",
        ...
        for itest in retrieval_test:
            ...
            for ilev in range(nlevel):
                ...
                axes.plot(latitude, temperature[ilev, :].T, lw=3)
                axes.grid()
...
def PlotRetrievedAerosolProfile():
    ...
    for iprior in prior:
        ...
        for itest in retrieval_test:
            ...
            for ilat in range(nlat):
                fig, axes = plt.subplots(1, prior_ncloud, figsize=(7, 10), sharex=True, sharey=True)        
