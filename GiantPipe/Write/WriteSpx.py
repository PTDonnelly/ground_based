...
def WriteBiDimSpx(dataset, mode, spectrals):
    """Create spectral input for NEMESIS using bi-dimensional binning scheme.
       Populate .spxfile with radiances, measurement errors, and geometries."""

    print('Creating bi-dimensional spectra...')

    def create_bidim_spx(f, lats, lons, mus, rads, rad_errs, waves):
        """Write spxfile for bi-dimensional binning method """
        # Write first line of texfile with relevant formatting
        clat = lats[0]
        clon = lons[0]
        nmu  = len(mus)
            ...
            if lats:
                if lons:
                    ...
                    with open(f"{dir}lat_{lats[0]}_lon_{lons[0]}.txt", 'w') as f:
                        create_bidim_spx(f, lats, lons, mus, rads, rad_errs, waves)
                    # Open spxfile
