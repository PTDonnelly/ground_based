...
def CalibrateMeridProfiles(Nfiles, single_merids, spectral_merids, wavenumber):
    ...
    for iwave in range(BinningInputs.nfilters):
        ...
        if ifilt_sc < 12:
            ...
            lmin_visir, lmax_visir = bn.nanmin(spectral_merids[:, ifilt_v, 0]), bn.nanmax(spectral_merids[:, ifilt_v, 0])
            lmin_calib, lmax_calib = bn.nanmin(cirs[:, ifilt_sc, 0]), bn.nanmax(cirs[:, ifilt_sc, 0])
            ...
            visirmean              = bn.nanmean(spectral_merids[visirkeep, ifilt_v, 3])
            # Use CIRS for N-Band
            calibkeep  = (cirs[:, ifilt_sc, 0] >= latmin) & (cirs[:, ifilt_sc, 0] <= latmax)
            calib      = cirs[:, ifilt_sc, 1]
            calibdata  = cirs[calibkeep, ifilt_sc, 1]
            calibmean  = bn.nanmean(calibdata)
        else:
            # Establish shared latitudes for accurate averaging
            lmin_visir, lmax_visir = bn.nanmin(spectral_merids[:, ifilt_v, 0]), bn.nanmax(spectral_merids[:, ifilt_v, 0])
            lmin_calib, lmax_calib = bn.nanmin(iris[:, ifilt_sc, 0]), bn.nanmax(iris[:, ifilt_sc, 0])
    ...
    for ifile, wave in enumerate(wavenumber):
        ...
        _, _, ifilt_sc, ifilt_v = SetWave(wavelength=False, wavenumber=wave)
        # Establish shared latitudes for accurate averaging
        lmin_single, lmax_single       = bn.nanmin(single_merids[:, ifile, 0]), bn.nanmax(single_merids[:, ifile, 0])
        lmin_spectral, lmax_spectral   = bn.nanmin(spectral_merids[:, ifilt_v, 0]), bn.nanmax(spectral_merids[:, ifilt_v, 0])
        ...
        singlemean                     = bn.nanmean(singledata)
        ...
        spectralmean                   = bn.nanmean(spectraldata)
